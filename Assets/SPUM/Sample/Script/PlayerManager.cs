...
{
    public bool _makeAutoUnit = true;
    ...
    public void ClearPlayerList()
    {
        ...
        if(!_makeAutoUnit)_savedUnitList.Clear();
        _playerList.Clear();
    }
    public void GetPlayerList()
    {
        if(_makeAutoUnit)
        {
            ...
            if(_random)
            {
                for(int i = 0; i < _savedUnitList.Count; i++)
                {
                    var temp = _savedUnitList[i];
                    int randomIndex = UnityEngine.Random.Range(i, _savedUnitList.Count);
                    _savedUnitList[i] = _savedUnitList[randomIndex];
                    _savedUnitList[randomIndex] = temp;
                }
            }
        }
